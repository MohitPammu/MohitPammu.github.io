...
(function() {
  ...
  window.SkillsSphereVisualization = {
    ...
    loadLogoForNode: function(iconUrl, parentNode) {
      ...
      if (isSVG) {
        // Chrome WebGL cannot handle SVGs directly — rasterize through canvas first
        const img = new Image();
        ...
        img.onerror = () => {
          console.warn(`Failed to load logo: ${iconUrl}`);
        };
        img.src = iconUrl;
      } else {
        // PNG/JPG — standard TextureLoader path works fine
