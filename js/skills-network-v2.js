// Skills Network Visualization using D3.js - Optimized version
document.addEventListener('DOMContentLoaded', function() {
    console.log('Initializing skills network visualization...');
    ...
    const config = {
        ...
        chargeStrength: -300,
        linkDistance: (d) => 200 - d.value * 20,
        collideRadius: (d) => d.size * 1.5,
        
        // Color themes
        lightColors: [
            'var(--primary-color)', // Primary color for group 1
            "#f45d48",             // For group 2
            "#ffd166",             // For group 3
            "#06d6a0",             // For group 4
            "#118ab2"              // For group 5
    ...
    function initSkillsNetwork() {
        // Check if the skills section container exists
        const skillsSection = document.getElementById('skills-network');
        if (!skillsSection) {
            console.warn('Skills network container not found');
            return;
        }
        ...
        const width = Math.min(containerWidth, config.maxWidth);
        ...
        const node = createNodes(svg, data.nodes, simulation);
        
        // Add labels to nodes
        addNodeLabels(node);
        
        // Add hover effects
        addHoverEffects(node, link, data);
        
        // Setup simulation ticks
        setupSimulationTicks(simulation, link, node);
        
        // Add legend
        addLegend(svg, width, height);
        
        // Append the SVG to the container
        skillsSection.appendChild(svg.node());
        
        // Add instruction text
        addInstructionText(skillsSection);
    }
    
    /**
     * Get skills data - organized skills with their relationships
     */
    ...
    function getThemeColor(index) {
        ...
        // Use the website's color variables when possible
        return isDarkTheme ? config.darkColors[index] : config.lightColors[index];
    }
    ...
    /**
     * Create force simulation with D3
     */
    function createForceSimulation(data, width, height) {
        return d3.forceSimulation(data.nodes)
            ...
            .force("collide", d3.forceCollide().radius(config.collideRadius));
    }
    ...
    /**
     * Add hover effects to nodes and links
     */
    function addHoverEffects(node, link, data) {
        node.on("mouseover", function(event, d) {
            ...
            d3.select(this).select("circle")
                ...
                .attr("r", d.size * config.hoverScaleFactor);
     ...
     * Setup simulation ticks
     */
    function setupSimulationTicks(simulation, link, node) {
        simulation.on("tick", () => {
            link
    ...
    /**
     * Add instruction text below the visualization
     */
    function addInstructionText(container) {
    ...
    document.addEventListener('themeChanged', function() {
        // Redraw the network when theme changes
        setTimeout(initSkillsNetwork, 300);
    });
