...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Wait for animations to complete before initializing components
    ...
    document.addEventListener('loadingComplete', () => {
        console.log('Loading animation complete, initializing components');
        initializeComponents();
    });
    
    // Fallback initialization if loading animation doesn't trigger the event
    if (document.body.classList.contains('loaded')) {
        console.log('Body already has loaded class, initializing components directly');
        initializeComponents();
    } else {
        // Set a fallback timeout in case loading animation fails
        setTimeout(() => {
            if (!window.componentsInitialized) {
                console.warn('Initializing components via fallback timeout');
                initializeComponents();
            }
    ...
    try {
        ...
        window.addEventListener('scroll', function() {
            try {
                ...
                // Sync with background if available
                if (typeof window.syncFlowingDataWithScroll === 'function') {
                    window.syncFlowingDataWithScroll(window.scrollY);
                }
            } catch (error) {
...
function initTypedText() {
    try {
        ...
        console.log('Typed text animation initialized');
    } catch (error) {
...
// Project filtering with improved error handling
...
// Update footer year with error handling
        ...
        contactForm.addEventListener('submit', function(e) {
            try {
                ...
                .then(data => {
                    ...
                    // Set a timer to fade out and show the form again
                    setTimeout(function() {
                        // Fade out
