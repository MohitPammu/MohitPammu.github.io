...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fix flash on initial load and header adjustment
    fixInitialLoadFlash();
});

/**
 * Preload State Handler
 * - Sets initial state of elements to prevent layout shifts
 */
function initPreloadState() {
    console.log('Initializing preload state...');
    
    // Set body class for preload state
    document.body.classList.add('preload');
    
    // Immediately set fixed positions for header elements to prevent layout shifts
    const header = document.querySelector('header');
    const logo = document.querySelector('.logo h1');
    
    if (header) {
        // Pre-calculate and set header dimensions
        header.style.height = '70px'; // Set fixed height for header
        header.style.transform = 'translateZ(0)'; // Hardware acceleration
        header.style.willChange = 'transform'; // Optimize for animations
        header.style.backfaceVisibility = 'hidden';
        header.style.transition = 'none'; // Disable transitions during initial load
    }
    
    if (logo) {
        // Ensure logo is positioned correctly from the start
    ...
    // Prevent hero content shifts by pre-positioning elements
    const heroContent = document.querySelector('.hero-content');
    const heroTitle = document.querySelector('.hero-content h1');
    const heroSubtitle = document.querySelector('.hero-content h2');
    
    if (heroContent) {
        // Ensure hero content has proper positioning from the start
        heroContent.style.opacity = '1';
        heroContent.style.transform = 'none';
        heroContent.style.position = 'relative';
        heroContent.style.transition = 'none';
        
        // Prevent text reflow by setting explicit positions
        if (heroTitle) {
    ...
    // Handle hero section visibility immediately
...
/**
 * Fix Initial Load Flash
 * - Handles the transition from preload to loaded state
 */
function fixInitialLoadFlash() {
    // Wait for full page load including images
    window.addEventListener('load', function() {
        console.log('Page fully loaded, transitioning to loaded state...');
...
/**
 * Project Filtering
 * - Handles the project filter buttons and animations
 */
...
/**
 * Filter Projects with Improved Animation
 * - Helper function for project filtering
 */
function filterProjects(cards, filterValue) {
    // Track if we need to re-layout the grid
    let needsLayout = false;
    
    // First pass: mark cards for visibility change
    cards.forEach(card => {
        // Check if card matches filter
        const shouldBeVisible = filterValue === 'all' || card.getAttribute('data-category') === filterValue;
        
        // Check if visibility will change
        if ((shouldBeVisible && card.classList.contains('hidden')) || 
            (!shouldBeVisible && !card.classList.contains('hidden'))) {
            needsLayout = true;
        }
    });
        ...
        setTimeout(() => {
            cards.forEach(card => {
                if (filterValue === 'all' || card.getAttribute('data-category') === filterValue) {
                    ...
                    setTimeout(() => {
                        card.classList.add('visible');
                    }, 50 * Math.random()); // Random stagger for natural effect
                } else {
